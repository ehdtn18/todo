...
// 비밀번호 표시용: 맨 앞 1 + 가운데 3 + 맨 뒤 1 = 총 5글자를 가린다.
export function maskFive(s) {
  s = String(s || "");
  const n = s.length;
  if (n <= 5) return "*".repeat(n);
  const hide = new Set([0, n - 1]);
  const start = Math.floor((n - 3) / 2);
  hide.add(start); hide.add(start + 1); hide.add(start + 2);
  return s.split("").map((ch, i) => (hide.has(i) ? "*" : ch)).join("");
}
