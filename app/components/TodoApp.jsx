...
function runApp(signal, created) {
  ...
  function EditorFactory(editorEl,menuEl,anchorEl){
    ...
    function tableEl(rows){const wrap=document.createElement("div");wrap.className="blk tableblock";wrap.contentEditable="false";let h='<table><tbody>';rows.forEach(function(r,ri){h+='<tr>';r.forEach(function(c){const tag=ri===0?"th":"td";h+='<'+tag+' contenteditable="true">'+esc(c)+'</'+tag+'>';});h+='</tr>';});h+='</tbody></table><div class="tbl-tools"><button type="button" data-tt="row-add">＋행</button><button type="button" data-tt="row-del">－행</button><button type="button" data-tt="col-add">＋열</button><button type="button" data-tt="col-del">－열</button></div>';wrap.innerHTML=h;return wrap;}
    ...
    // 표 행/열 편집. afterIdx==null이면 마지막 기준.
    let tblCell=null;
    function tblAddRow(table,after){const n=table.rows[0]?table.rows[0].cells.length:1;const at=Math.min((after==null?table.rows.length-1:after)+1,table.rows.length);const tr=table.insertRow(at);for(let k=0;k<n;k++){tr.insertCell().contentEditable="true";}return tr;}
    function tblDelRow(table,idx){if(table.rows.length<=1)return;table.deleteRow(Math.min(idx==null?table.rows.length-1:idx,table.rows.length-1));const first=table.rows[0];if(first)Array.prototype.forEach.call(first.cells,function(c){if(c.tagName==="TD"){const th=document.createElement("th");th.contentEditable="true";th.innerHTML=c.innerHTML;c.replaceWith(th);}});}
    function tblAddCol(table,after){const cols=table.rows[0]?table.rows[0].cells.length:0;const at=(after==null?cols-1:after)+1;Array.prototype.forEach.call(table.rows,function(r,ri){const c=document.createElement(ri===0?"th":"td");c.contentEditable="true";r.insertBefore(c,r.cells[at]||null);});}
    function tblDelCol(table,idx){const cols=table.rows[0]?table.rows[0].cells.length:0;if(cols<=1)return;const ci=idx==null?cols-1:idx;Array.prototype.forEach.call(table.rows,function(r){if(r.cells[ci])r.deleteCell(ci);});}
    function selCell(){const s=window.getSelection();if(!s||!s.rangeCount)return null;let n=s.anchorNode;n=(n&&n.nodeType===3)?n.parentNode:n;const c=n&&n.closest?n.closest("th,td"):null;return (c&&c.closest(".tableblock"))?c:null;}
    editorEl.addEventListener("focusin",function(e){const c=e.target.closest&&e.target.closest("th,td");if(c&&c.closest(".tableblock"))tblCell=c;});
    editorEl.addEventListener("click",function(e){const btn=e.target.closest(".tbl-tools button");if(!btn)return;e.preventDefault();const wrap=btn.closest(".tableblock"),table=wrap.querySelector("table");const cell=(tblCell&&wrap.contains(tblCell))?tblCell:null;const ri=cell?cell.parentNode.rowIndex:null,ci=cell?cell.cellIndex:null,a=btn.dataset.tt;if(a==="row-add")tblAddRow(table,ri);else if(a==="row-del")tblDelRow(table,ri);else if(a==="col-add")tblAddCol(table,ci);else if(a==="col-del")tblDelCol(table,ci);histRecord();updateEmpty();});
    ...
    editorEl.addEventListener("keydown",function(e){
      ...
      if(e.key==="Tab"){const sc=selCell();if(sc){e.preventDefault();const table=sc.closest("table");const cells=Array.prototype.slice.call(table.querySelectorAll("th,td"));const idx=cells.indexOf(sc);let nx=cells[idx+(e.shiftKey?-1:1)];if(!nx&&!e.shiftKey){tblAddRow(table,table.rows.length-1);histRecord();nx=table.querySelectorAll("th,td")[idx+1];}if(nx)placeCaret(nx,true);return;}}
  ...
  function pushNoti(t,sub,tag){notiflog.unshift({nid:uid(),taskId:t.id,title:t.title||"(제목 없음)",sub:sub,tag:tag||"",at:Date.now(),seen:false});if(notiflog.length>50)notiflog.length=50;saveNotiflog();paintBell();try{if(typeof Notification!=="undefined"&&Notification.permission==="granted")new Notification((tag?"("+tag+") ":"")+(t.title||"할 일"),{body:sub});}catch(e){}}
  function fireNoti(t){pushNoti(t,"마감일 알림 · "+(labelRangeT(t.start,t.end,t.startTime,t.endTime)||""),"완료");}
  function fireConfirmNoti(t,cf,n){pushNoti(t,n+"차 컨펌일 · "+fmtMD(cf.date),n+"차");}
  ...
  function renderNoti(){if(!notiflog.length){$("notiList").innerHTML='<div class="empty" style="padding:30px 10px"><p>알림이 없어요</p></div>';return;}$("notiList").innerHTML=notiflog.map(function(x,i){const d=new Date(x.at);const ts=(d.getMonth()+1)+"/"+d.getDate()+" "+pad(d.getHours())+":"+pad(d.getMinutes());return '<div class="trashitem noti'+(x.seen?"":" unseen")+'" data-tid="'+esc(x.taskId)+'"><div class="ti-main"><div class="ti-title">'+(x.tag?'<span class="noti-tag'+(x.tag==="완료"?" done":"")+'">'+esc(x.tag)+'</span>':"")+esc(x.title)+'</div><div class="ti-sub">'+esc(x.sub)+' · '+ts+'</div></div><div class="ti-acts"><button class="iconbtn ndel" data-ndel="'+i+'" title="알림 삭제" aria-label="알림 삭제"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg></button></div></div>';}).join("");}
