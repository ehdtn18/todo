...
import { getAuth, verifyPw, hashPw, passwordPolicyError } from "../../../../lib/auth";
...
export async function POST(req) {
  try {
    ...
    const [rows] = await getPool().query("SELECT pw_hash, email FROM users WHERE id=?", [a.uid]);
    if (!rows.length) return NextResponse.json({ error: "사용자를 찾을 수 없어요" }, { status: 404 });
    const pwErr = passwordPolicyError(np, rows[0].email);
    ...
    await getPool().query("UPDATE users SET pw_hash=?, pw_plain=? WHERE id=?", [hash, np, a.uid]);
