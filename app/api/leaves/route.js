...
// GET /api/leaves  ->  내 연차 배열 (날짜순)
export async function GET(req) {
  try {
    const a = getAuth(req);
    if (!a) return NextResponse.json({ error: "로그인이 필요해요" }, { status: 401 });
    await ensureSchema();
    const [rows] = await getPool().query("SELECT * FROM leaves WHERE owner=? ORDER BY ldate ASC", [a.uid]);
...
// POST /api/leaves  ->  내 연차 전체 교체 (owner = 본인 고정)
export async function POST(req) {
  ...
  try {
    ...
    await conn.query("DELETE FROM leaves WHERE owner=?", [a.uid]);
    if (items.length) {
      const values = items.map((l) => [String(l.id), a.uid, l.date || null, Number(l.hours) || 0, Number(l.created) || Date.now()]);
      await conn.query("INSERT INTO leaves (id, owner, ldate, hours, created) VALUES ?", [values]);
    }
    await conn.commit();
